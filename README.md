...
- **Shorts 短视频模式** — 独立的短视频浏览页面，支持上下滑动切换、左滑呼出视频列表侧边栏、文件夹内播放时推荐其他文件夹视频；播放页可一键进入短视频模式
- **视频 API** — 提供 JSON 格式的视频列表接口（`/api/videos`），支持分页和直播格式过滤
- **目录元数据** — 在视频目录下放置 `metadata.json` 自定义视频标题、来源和描述
